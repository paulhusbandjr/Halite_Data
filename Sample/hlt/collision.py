...
def intersect_segment_circle(start, end, circle, *, fudge=0.5):
    ...
    logging.info("\nin intersect_segment_circle, start: {}\nend: {}\n, circle: {}".format(start, end, circle))
    dx = end.x - start.x
    dy = end.y - start.y
    ...
    if a == 0.0:
        ...
        ans = start.calculate_distance_between(circle) <= circle.radius + fudge
        logging.info("\nreturned {}".format(ans))
        return ans
    ...
    t = min(-b / (2 * a), 1.0)
    if t < 0:
        logging.info("\nreturned False, no intersection")
        return False
    ...
    ans = closest_distance <= circle.radius + fudge
    logging.info("\nreturned {}".format(ans))
    return ans
